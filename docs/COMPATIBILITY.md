...
| `android-clipboard-history` | format | `2` | authenticated v1 migrates atomically to v2 with all entries unpinned; exact v2 preserves pin state; unknown outer or inner future versions are preserved but hidden, while malformed current ciphertext/payload fails closed | same backup-excluded private AES-256-GCM domain; at most 100 distinct entries and 120,000 code points; older v1 builds preserve but cannot display v2; no plaintext fallback or downgrade writer | `android/app/src/main/java/com/opentypeless/android/keyboard/clipboard/ClipboardHistoryCodec.java`, `android/app/src/main/java/com/opentypeless/android/keyboard/clipboard/ClipboardHistoryStore.java`, `android/app/src/main/java/com/opentypeless/android/security/LocalClipboardCipher.java` | `KBD-011` |
| `android-emoji-recents` | format | `1` | exact v1 only; malformed, oversized, over-deep, duplicate and out-of-catalog values fail closed to a bounded empty/filtered MRU | private backup-excluded code-point payload; at most 21 catalog IDs; sensitive/no-learning fields neither read nor write it | `android/app/src/main/java/com/opentypeless/android/keyboard/emoji/EmojiRecentCodec.java`, `android/app/src/main/java/com/opentypeless/android/keyboard/emoji/EmojiRecentStore.java` | `KBD-010` |
| `android-emoji-catalog` | data | `unicode15.1,cldr45` | checked-in generated source only; maintenance generation rejects input-hash, count, duplicate, annotation and bound drift | 1,898 fully-qualified base sequences and bounded English/Chinese annotations; no runtime parser, download, font or artwork | `android/app/src/main/java/com/opentypeless/android/keyboard/emoji/EmojiCatalogData.java`, `scripts/generate_emoji_catalog.py` | `KBD-010` |
