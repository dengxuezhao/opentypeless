...
**KBD-008 个人回归修复（2026-08-22，内部小鹤优先）：** 上述外部输入法短按兜底不再用于 Route-A 语言槽。
本地 Rime 可用时底栏只显示内部 `EN` / `中` 引擎切换；本地包缺失时显示“导入中文”，短按打开应用私有的显式
SAF 导入页，长按才打开系统输入法选择器。小米 10 Ultra 已重新导入用户原有 `LOCAL_ONLY` 小鹤包，并在系统设置
一次性搜索字段中验证内部引擎切换、候选显示及候选提交；测试文本随后清除。真实资源未进入 Git 或 APK。
